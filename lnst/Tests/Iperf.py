...
class IperfServer(IperfBase):
    ...
    def _compose_cmd(self):
        ...
        if "cpu_bind" in self.params and len(self.params.cpu_bind):
            cpu = ','.join(str(cpu) for cpu in self.params.cpu_bind)
        else:
            cpu = ""
        ...
        cmd = "taskset -c {cpu} iperf3 -s {bind} -J {port} {oneoff} {mptcp} {opts}".format(
                cpu=cpu,
...
class IperfClient(IperfBase):
    ...
    def _compose_cmd(self):
        ...
        cmd = ("taskset -c {cpu} iperf3 -c {server} -b 0/1000 -J -t {duration}"
