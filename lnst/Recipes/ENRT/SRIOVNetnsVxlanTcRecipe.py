...
class SRIOVNetnsVxlanTcRecipe(
    ...
    def add_network_layers(self, config):
        ...
        for host in [host1, host2]:
            ...
            for dev in self.sriov_devices[host][0]:
    ...
    def add_tc_filter_rules(self, config):
        ...
        # tc configuration
        for host in [host1, host2]:
            vf_representor = self.sriov_devices[host].vf_reps[0]
    ...
    def dump_tc_rules_devices(self):
        return [dev for host in self.matched for dev in [host.vxlan10, self.sriov_devices[host].vf_reps[0]]]
