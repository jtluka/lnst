...
class LinuxPerfMeasurement(BaseMeasurement):
    ...
    @property
    def configurations(self) -> Iterable[tuple[int, Host, list[int]]]:
        for host in self.hosts:
            for cpu_list_id, cpu_list in enumerate(self.linuxperf_cpus):
                yield (cpu_list_id, host, cpu_list)

    def start(self) -> None:
        self._start_timestamp = time.time()
        for cpu_list_id, host, cpus in self.configurations:
    ...
    def finish(self) -> None:
        for job, (_, host, _) in zip(self._jobs, self.configurations):
    ...
    def collect_results(self) -> list[BaseMeasurementResults]:
        ...
        for job, (_, host, cpus) in zip(self._jobs, self.configurations):
